...
const Seller = () => {
  ...
  const [form, setForm] = useState({
    name: "", type: "", size: "", quality: "", condition: "", price: "", company: "", image: "", description: "", location: "",
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    addProduct({ ...form, price: isDonation ? "0" : form.price, isDonation });
    setSubmitted(true);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input name="name" value={form.name} onChange={handleChange} placeholder="e.g. Cotton T-Shirt" required
            ...
            <div>
              ...
              <input name="size" value={form.size} onChange={handleChange} placeholder="e.g. M, L, 32" required
                className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">Quality</label>
              <input name="quality" value={form.quality} onChange={handleChange} placeholder="e.g. Good, Like New" required
                className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">Brand / Company</label>
              <input name="company" value={form.company} onChange={handleChange} placeholder="e.g. EcoWear" required
                className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">Location</label>
              <input name="location" value={form.location} onChange={handleChange} placeholder="e.g. Mumbai, Delhi" required
                className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            {!isDonation && (
              <div>
                <label className="mb-1 block text-sm font-medium text-foreground">Price (₹)</label>
                <input name="price" value={form.price} onChange={handleChange} placeholder="e.g. 500" required={!isDonation} type="number" min="1"
                  className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
            )}
          </div>
